...
NoiseMatrix = 0:0.1:3;
ErrorMatrix = zeros(1,length(NoiseMatrix));

for b = 1:length(NoiseMatrix)
    Noise = NoiseMatrix(1,b);
...
OptKMatError = (OptKMatrix - KMatrix)/norm(KMatrix);
NormOptKMatError = norm(OptKMatError);
...
%% Plot effect of noise on error of EstimatedKMatrix 
 ...
 figure (2)
 plot( NoiseMatrix, ErrorMatrix, 'LineWidth', 0.9)
 title('Error of OptKMatrix w.r.t Noise ')
 xlabel('Noise variance')
 legend('y = ErrorOptKMatrix', 'Location','southwest')
p = polyfit(NoiseMatrix,ErrorMatrix,1);
f = polyval(p,NoiseMatrix);
hold on
plot(NoiseMatrix,f,'--r')

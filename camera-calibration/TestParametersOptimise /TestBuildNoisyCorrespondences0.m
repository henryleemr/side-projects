function [Correspondence] = BuildNoisyCorrespondences(Correspondences,n,CameraHeight,CameraWidth, CameraScale)
...
Correspondences = Correspondences + Noise;
...
Correspondence = FilteredCorrespondence(:,1:points);
TruncatedCorrespondence = Correspondence;
%%
% Test the results

figure (2) % new figure
ax1 = subplot(3,3,1); % u,v only without noise
ax2 = subplot(3,3,2); % u,v only with noise
ax3 = subplot(3,3,3); % u,v, truncated only
figure (1)
ax4 = subplot(3,3,1);  % u,v,x,y without noise
ax5 = subplot(3,3,2); % u,v,x,y with noise
ax6 = subplot(3,3,3);  % u,v,x,y truncated   
plot(ax2, Correspondences(1 ,:) ,Correspondences(2 ,:) ,'.')
title(ax2, 'u,v WITH noise ')
axis ij
plot(ax5, [Correspondences(1 ,:) Correspondences(3 ,:)] ,[Correspondences(2 ,:) Correspondences(4 ,:)] ,'.')
title(ax5,'u,v,x,y WITH noise ')
axis ij
plot(ax1, NoiselessCorrespondences(1 ,:) ,NoiselessCorrespondences(2 ,:) ,'.')
title(ax1,'u,v WITHOUT noise ')
axis ij
plot(ax4, [NoiselessCorrespondences(1 ,:) NoiselessCorrespondences(3 ,:)] ,[NoiselessCorrespondences(2 ,:) NoiselessCorrespondences(4 ,:)] ,'.')
title(ax4,'u,v,x,y WITHOUT noise ')
axis ij
plot(ax3, TruncatedCorrespondence(1 ,:) ,TruncatedCorrespondence(2 ,:) ,'.')
title(ax3,'Truncated u,v ')
plot(ax6, [TruncatedCorrespondence(1 ,:) TruncatedCorrespondence(3 ,:)] ,[TruncatedCorrespondence(2 ,:) TruncatedCorrespondence(4 ,:)] ,'.')
title(ax6,'Truncated u,v,x,y ')
